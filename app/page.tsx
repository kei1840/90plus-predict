...
export default function Home() {
  ...
  return <main>
    ...
    <div className="layout">
      ...
      <aside className="panel ranking" id="ranking"><div className="section-title"><span>02</span><div><h2>ライブランキング</h2><p>結果を入力すると自動で採点</p></div></div>
        <div className="rank-list">{ranked.length===0?<div className="empty"><b>一番乗りしよう</b><p>予想を登録すると、ここにランキングが表示されます。</p></div>:ranked.map((p,i)=><article key={p.id} className={i<3?`top top-${i+1}`:""}><div className="place">{i+1}</div><div className="avatar">{p.name.slice(0,1)}</div><div className="person"><b>{p.name}</b><small>決勝 {p.finalSpain}–{p.finalArgentina} · 3位 {p.bronzeFrance}–{p.bronzeEngland}</small></div><strong>{p.score}<small> pts</small></strong></article>)}</div>
